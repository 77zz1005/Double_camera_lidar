# RM_Radar2023
### 开发日志
Date-5.2:完成裁判系统通信的修改与测试

#### 双相机方案
1.单相机-->双相机
   （1）如何区分左右相机
   解决方案：通过程序输出不同相机的产品序号，确定唯一相机，最后贴标签纸
   MV_Camera是一起初始化的，构造函数会输出相关内容
   进度：待测试
   （2）双相机pnp标定界面
   解决方案：按顺序标，先左后右
   进度：待测试
   （3）推理
   进度：待测试--重点在于双相机的推理速度会有多慢
   （4）融合识别信息
   PLAN_A通过识别框的id与像素距离判断是否是同一物体，在第一次深度信息获取时修改
   PLAN_B通过识别框的id与点云深度判断是否是同一物体，在第二次深度信息获取时修改
   验证方案：目前计划利用同一个视频，但是读取2次，假设它为左右相机的视角，以检查融合识别框是否有逻辑错误
   * 右相机效果可能更好，如何融合左右相机识别框的世界坐标信息？重复id/无重复id
   TODO:视频录制GUI ：this->nh->setParam("/radar2023/Recorder", false);
       bool VideoRecorderSiginal = false;
...
[识别]
* 可以识别到平衡步兵
*红3-cls2-映射8-8+1=9=R3 
*蓝5-cls8-映射2-2+1=3=B3
3.测试deepsort跟踪效果
